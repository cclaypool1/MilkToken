...
contract ButterVotingNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    
    uint256 private currentId = 0;
        
    string[3] tokenURIs;
    
    uint256 tier1cuttoff;
    uint256 tier2cuttoff;
    
    IButterVoting votingContract;
        
    mapping(uint256 => uint256) private tiers;
    
    mapping(address => bool) private claimedNFT;
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        return tokenURIs[tiers[tokenId]];
    }

    function claim() public nonReentrant {
        //read from the voting contract to determine which tier to give
        require(votingContract.hasVotedAddress(msg.sender), "You must vote to claim an NFT");
        require(!claimedNFT[msg.sender], "You have already claimed an NFT");
        
        uint256 voteCount = votingContract.addressVoteWeightAddress(msg.sender);
         if(voteCount > tier1cuttoff)
        {
            tiers[currentId] = 1;
        }
        if(voteCount > tier2cuttoff)
        {
            tiers[currentId] = 2;
        }
    ...
    function setURIs(string memory tier1uri, string memory tier2uri, string memory tier3uri) public onlyOwner()
    {
        tokenURIs[0] = tier1uri;
        tokenURIs[1] = tier2uri;
        tokenURIs[2] = tier3uri;
    }
    ...
    {
        return claimedNFT[msg.sender];
    }
    
    
    constructor(string memory tier1uri, string memory tier2uri, string memory tier3uri, uint256 pollId, uint256 series, address votingContractAddress, uint256 tier1cut, uint256 tier2cut) ERC721(string(abi.encodePacked("Butter Voting NFT Series ", toString(series))), string(abi.encodePacked("BVOTEV3S", toString(series)))) Ownable()
    {
        tokenURIs[0] = tier1uri;
        tokenURIs[1] = tier2uri;
        tokenURIs[2] = tier3uri;
        ...
        poll = pollId;
        tier1cuttoff = tier1cut;
        tier2cuttoff = tier2cut;
    }
}
...
contract ButterVoting is Context, Ownable, IButterVoting {
    ...
    mapping (uint256 => Poll) completedPolls;
    uint256 pollNumber = 8;
    uint256 series = 6;
    ...
    function startNewPoll(string memory tier1uri, string memory tier2uri, string memory tier3uri, uint256 tier1cut, uint256 tier2cut) public onlyOwner
    {
        //Clear charity list
        ...
        votingNFT = new ButterVotingNFT(tier1uri, tier2uri, tier3uri, pollNumber, series, address(this), tier1cut, tier2cut);
        series = series.add(1);
    }
    ...
    function endPoll() public onlyOwner
    {
        ...
        for(uint256 i = 0; i < charities.length; i++)
        {
            ...
            completedPolls[pollNumber].charities.push(charities[i]);
        }
    ...
    function claimed() internal view returns (bool)
    {
        return votingNFT.claimed();
    }
    
    function getPreviousPoll(uint256 poll) public view returns (Charity[] memory)
    {
        return completedPolls[poll].charities;
    }
