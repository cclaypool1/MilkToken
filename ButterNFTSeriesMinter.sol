...
contract SeriesMinter is Context
{
    ...
    //pricing
    uint256 creationPrice = 60000000000000000;
    ...
    function createCollection(string memory tokenname, string memory tokensymbol, string memory tokenuri, uint256 price, uint256 max) public payable
    {
        if(msg.sender != _owner)
        {
            require(msg.value == creationPrice, "Incorrect message value");
        }
        //process fees
        uint256 paidBNB = msg.value;
        uint256 quarter = paidBNB.div(4);
        
        uint256 milkQuarter = quarter;
        uint256 butterQuarter = quarter;
        uint256 milkLPQuarter = quarter;
        uint256 butterLPQuarter = paidBNB.sub(milkQuarter).sub(butterQuarter).sub(milkLPQuarter);
        ...
        if(msg.sender != _owner)
        {
            ...
            uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value:butterQuarter}(
            ...
            //now add the liquidity for Milk and Butter and burn
            butter.approve(address(uniswapV2Router), collectedButter);
...
contract ButterMintNFTSeries is Context, ERC165, IERC721, IERC721Metadata {
    ...
    uint256 purchasePrice = 10000000000000000;
    ...
    function buyWithButter() public
    {
        ...
        uint256 collectedButter = butter.balanceOf(address(this));
        butter.transferFrom(msg.sender, address(this), salePriceButter(purchasePrice));
        collectedButter = butter.balanceOf(address(this)).sub(collectedButter);
        butter.transfer(creator, collectedButter);
    ...
    function buyWithMilk() public
    {
        ...
        uint256 collectedMilk = milk.balanceOf(address(this));
        milk.transferFrom(msg.sender, address(this), salePriceMilk(purchasePrice));
        collectedMilk = milk.balanceOf(address(this)).sub(collectedMilk);
    ...
    function buyWithBNB() public payable
    {
        ...
        require(sellingActive, "This sale is not running");
        require(msg.value >= purchasePrice, "Insuffienct BNB value");
        ...
        //send that butter to the seller
        butter.transfer(creator, transferableButter);
    ...
    constructor (string memory name_, string memory symbol_, string memory uri_, address minter, uint256 price, uint256 maxMint) {
        _name = name_;
        _symbol = symbol_;
        URI = uri_;
        creator = minter;
